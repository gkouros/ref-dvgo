...
data = dict(
    ...
    llff_bd_factor=0.75,          # scale multiplier for rendered poses
    blender_radius=4.0,           # radius of rendered poses
    ...
    pose_noise_mag=0.0,           # multiplier of gaussian distribution N~(0, 1)
)
...
coarse_train = dict(
    ...
    lrate_grad_pred=1e-1,         # lr of density gradient prediction voxel grid
    lrate_diffuse=1e-1,           # lr of density gradient prediction voxel grid
    lrate_tint=1e-1,              # lr of tint voxel grid
    lrate_roughness=1e-1,         # lr of roughness voxel grid
    ...
    weight_nearclip=0,            # weight of near plane clipping loss
    weight_distortion=0,          # weight of distortion loss
    ...
    weight_tv_grad_pred=0.0,      # weight of total variation loss of grad pred voxel grid
    weight_tv_tint=0.0,           # weight of total variation loss of specular tint voxel grid
    weight_tv_roughness=0.0,      # weight of total variation loss of roughness voxel grid
    weight_tv_diffuse=0.0,        # weight of total variation loss of diffuse voxel grid
...
coarse_model_and_render = dict(
    ...
    enable_pred_normals=False,    # Enables predicted normals voxel grid
    use_specular_tint=False,      # Enables specular tint grid
    use_reflections=False,        # Enables reflection-based reparametrization
    use_refnerf_architecture=False,
    use_n_dot_v=False,            # Enables feeding the dir. MLP with n.v
    normals_to_use='normals_pred',# Select which normals to use for reflection
    disable_density_normals=True, # disable density gradient based normals
    enable_pred_roughness=False,  # Enable predicted roughness
    use_sh_encoding=False,        # Enables spherical harmonics encoding
    deg_view=5,                   # the degrees of the spherical harmonics
    density_config=dict(),
    k0_config=dict(),
    ...
    use_gradient_scaler=False,    # enables gradient scaling to reduce floaters from background collapse
    new_act_shift=False,          # if True use eq. 9 from the paper
)
