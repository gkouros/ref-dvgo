...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 ...
                 viewbase_pe=4, new_act_shift=False,
                 **kwargs):
        ...
        if rgbnet_dim <= 0:
            ...
            dim0 = (3+3*viewbase_pe*2) + self.k0_dim * (not self.rgbnet_full_implicit)
        ...
        # diffuse color voxel grid
        self.use_diffuse_color = not self.rgbnet_direct or self.rgbnet is None
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        if self.use_diffuse_color:
            diffuse = self.diffuse(ray_pts)

        if self.rgbnet is None:
            # no view-depend effect
            rgb = torch.sigmoid(diffuse)
        else:
            # view-dependent color emission
            viewdirs_emb = (viewdirs.unsqueeze(-1) * self.viewfreq).flatten(-2)
            viewdirs_emb = torch.cat([viewdirs, viewdirs_emb.sin(), viewdirs_emb.cos()], -1)
            viewdirs_emb = viewdirs_emb.flatten(0,-2)[ray_id]
            rgb_feat = [viewdirs_emb]

            # query for appearance features
            if not self.rgbnet_full_implicit:
                k0 = self.k0(ray_pts)
                rgb_feat.append(k0)

            # query for directional mlp logit
            ...
            if self.rgbnet_direct:
                ...
                rgb = torch.sigmoid(rgb_logit + diffuse)

            if render_kwargs.get('render_features', False):
                features_marched = segment_coo(
                    src=(weights.unsqueeze(-1) * k0),
                    index=ray_id,
                    out=torch.zeros([N, k0.shape[1]]),
                    reduce='sum')
        ...
        if self.use_diffuse_color and render_kwargs.get('render_diffuse', False):
            with torch.no_grad():
                diffuse_marched = segment_coo(
                    src=(weights.unsqueeze(-1) * torch.sigmoid(diffuse)),
                    index=ray_id,
                    out=torch.zeros([N, 3]),
                    reduce='sum')
